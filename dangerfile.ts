...
const config: DangerConfig = {
	...
	TITLE: {
		PATTERN: new RegExp(
			`^(${ProjectPrefix.CHANGE_TYPES.join("|")})(\\((${ProjectPrefix.SCOPES.APPS.join("|")}|${ProjectPrefix.SCOPES.PACKAGES.join("|")})(\\/(${ProjectPrefix.SCOPES.APPS.join("|")}|${ProjectPrefix.SCOPES.PACKAGES.join("|")}))*\\))?: (.*\\S) tm-[0-9]{1,6}$`,
		),
	},
};
